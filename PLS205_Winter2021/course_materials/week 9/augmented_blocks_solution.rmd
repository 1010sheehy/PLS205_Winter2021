...
```{r}
missing_lm <- lm(yield ~ block + gen, data = soybean_missingBlocks)
missing_means <- emmeans(missing_lm,specs = 'gen',at=list(gen = c('G03','G06','G07','G11','G12')))
missing_contrasts <- contrast(missing_means,method = 'pairwise')
summary(missing_contrasts,infer = c(T,F))
```
...
```{r}
unconnected_data = read.csv('unconnected.csv',colClasses = c('factor','factor'), fileEncoding = "UTF-8-BOM")
str(unconnected_data)
```
The argument " fileEncoding = "UTF-8-BOM" " was added in order to get rid of the "ï.." from the Block term.  
