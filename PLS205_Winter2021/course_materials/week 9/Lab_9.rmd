...
```{r}
soybeans = read.csv('soybean.csv',stringsAsFactors = TRUE)
str(soybeans)
```

### Visualsing the design
With a blocked design, a table that counts the number of occurrences of each treatment in each block 
can help visualize the experiment. This is called an **incidence matrix** for the design
The `is_crossed` function counts the number of occurrences of each combination of the listed factors
and return the incidence matrix which you can store in a variable and print:
```{r}
...
incidence_mat_lost_blocks = is_crossed(block~gen,soybeans_lostBlocks,only_full_crossed = TRUE)  # adding only_full_crossed = TRUE will only return TRUE if every block contains every genotype.
```
...
#### Random blocks
In a block design, we declare block:treatment to be random if it is included in our model formula because this is the experimental unit for treatment.
...
However, in an incomplete block design, declaring blocks to be random 
...
```{r}
# write.csv(augblock,file = 'augmented_blocks.csv',row.names=F)
augblock <- read.csv('augmented_blocks.csv',stringsAsFactors = TRUE)
str(augblock)
```
...
In this case, maybe not. The ANOVA test on Var asks if ANY variety differs from any other (like the Tukey test).
