...
> Fill in your scores in the google form here: https://forms.gle/T4kK8gwrxjwj4t8W7. 
...
|-----------|----------|-------------|----------|-----------------|
| Treatment | gen      | Categorical | 24       | rep:gen         |
| Design    | rep      | Categorical | 3        |                 |
|           | block    | Categorical | 18       |                 |
|           | rep:gen  | Categorical | 72       |                 |
|           | block:gen| Categorical | 72       |                 |
|           | plot     | Categorical | 72       |                 |
...
> [2 point] For explanation on whether or not the design is balanced, based on your lambda value.
...
```{r}
crossprod(is_crossed(block:rep~gen,oats))
```
...
> [3 points] For your explanation of why the cross product matrix shows that the design is unbalanced. 
...
> [3 points] For showing that each rep (field) has 1 replicate of each genotype. This can be through a cross product matrix, a table, etc.
> [3 points] For mentioning something about how the reps (fields) are resolvable. 
...
> [3 points] For the correct model formulation. There are potentially two ways to model this, using a random effects model (lmer) with blocks nested within reps (block:rep), or using an lm model with a variable that is unique for each block*rep combination (block2). 
...
> [3 points] For your explanation based on your model. If you modeled according to the answer key, then you should mention that there is strong evidence for the difference in genotypes for yield.
...
```{r}
means = emmeans(lme1,specs = 'gen',ddf='K',level = 0.99)
# means = emmeans(lm(yield ~ rep + block2 + gen,oats),specs = 'gen')
cld(means,alpha = 0.1,level = 0.9)
```
...
> [3 points] For the cld. Your emmeans object should also be using an alpha 0.1, not 0.1.
> [3 points] You need to also mention that only a few genotypes can be distinguished from the genotype with the highest mean (in this case G03, G09, G20 and G07 are distinguishable from G01 an alpha of 0.1) for full credit. Conversely, you can mention the genotypes that aren’t distinguishable. You need to get all of the distinguishable and/or undistinguishable genotypes for full credit. Otherwise [-1 point]
...
> [6 points] For mentioning that the confidence intervals are wider for some comparisons than others. You can also say how certain combinations of genotypes are not observed in the same block, which increases our uncertainty of those comparisons.
