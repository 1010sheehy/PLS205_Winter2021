...
> Fill in your scores in the google form here: https://forms.gle/cSThQoyA7mvWoTKd6
...
> The soil samples are subsamples because the treatments were applied randomly to the plots, so the four soil samples from that plot all received the same application of a treatment. Therefore, soil-sample couldn't be the EU. Since each plot received only only level of Strain, it can be an EU.
>
> [4 points] Must mention the randomization and the experimental unit
...
> [4 points] The key is the Plot and the # levels. The "Type" column is not very important. It could be "Factor" instead of "Cat".
...
> [5 points] 3 points for the correct code and answers, 2 for comparison to lab
...
> [6 points]: 2 for correct model, 2 points for QQ-plot and 2 points for a plot that shows variances between groups.
...
summary(drug_effects,level = 0.95,infer = c(T,T))
```

> There is fairly strong evidence that drugs A and D improve on the old standard. The 95% confidence intervals
do not cross zero, and the p-values are fairly small. You could also say there is weak evidence that B also improves because its p-value is also pretty small.

> [5 points]: We are looking specifically for comparisons between drugs A-D and E (the old standard). 
> 3 points for the pairwise comparisons, and 2 points for discussing how A and D show strong evidence for being better than drug E.
...
> [5 points]: You need to show some sort of comparison between the drugs, not just between drugs A-D and the old standard.
...
> [5 points]: To get full credit you need to mention all of the above: differences in SE, and asking multiple questions/lack of control for multiple comparisons.
...
> 3 points for the explanation. If you are missing any of the parts above (SE is different, asking multiple questions/lack of control for multiple comparisons) then -1 point
