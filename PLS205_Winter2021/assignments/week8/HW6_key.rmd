...
> Fill in your scores in the google form here: https://forms.gle/bhBBeTPWnL9Ms4QF9. 
...
>[8 points] [-2 points] For any incorrect # levels or wrong experimental units. Section is not needed, but Field:Species is because it is the EU.
...
pls205_diagnostics(lm1)
```

> Of these, the Scale-Location plot is the most suspicions, showing a clear "U" pattern as discussed in lab. This suggests that there may be a Field:Species multiplicative interaction which would mean the Species effects are not constant across fields. This isn't necessarily a problem for RCBD analyses, but is something to keep track of. Unfortunately, log-transforming the data does not seem to help much - the diagnostics look very much the same.

> In this case, how should you chose? The answer is really about clarity or presentation. Sometimes it makes more sense to talk about fold-changes, and sometimes about absolute changes. If the transformation doesn't make much difference in the diagnostics, chose whichever you'd prefer to discuss, or whichever makes more sense in terms of the mechanism by which the treatment operates. There shouldn't be any particular reason to favor transformed or un-transformed data otherwise. For example, if you'd like to talk about % changes of fold-changes, just do the analysis of log-transfored data.
...
```{r}
block_interactions_plot(lm1,block = 'Field')
```
...
>[7 points] You need to give a reason for why you think a RCBD or CRD works better. For instance, you can say that CRD potentially works better because Fields is not relatively important (showing that the p-value for Fields is not significant at alpha = 0.05). A more complete answer would follow what's written above, where you calculate the F_crit values for both the RCBD and CRD. This is not needed for full credit.
...
> The importance of blocking now increased. The MS_Field increased (because the fields are now more variable). The SS and MS Species did not change, nor did the F-value because the experimental error (MSE) didn't change. However, if we now ignored the Fields and analyzed as a CRD, our MSE would go up more than in the original data (due to larger SS_BLock), meaning that the effect of Block on the Species F-value becomes more important.
