...
| Structure | Variable   | Type        | # levels | EU        |
|-----------|------------|-------------|----------|-----------|
| Treatment | trt        | categorical | 6        | field:trt | # because field:trt is crossed with block, may need to add field:trt:block term
...
# the model
apples_2_model <- lmer(yield ~ (1|Field:trt) + (1|block) + Field + trt, data = apples_2) #block might not be declared because not nested within field:treatment --> IS crossed

is_nested(Field:trt ~ block, apples_2 )
is_crossed(Field:trt ~ block, apples_2)
```
...
```
> No, the general conclusion is still the same -- E has a significantly higher yield than the standard treatment, S. COMMENT ON PRECISION AND HOW INTERPRETATION HAS CHANGED.
