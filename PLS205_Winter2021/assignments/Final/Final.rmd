...
```
> There is strong evidence that nitrogen has an effect on biomass (p = 0.0002253). Additionally, the F value is relatively high (F value = 23.240) meaning that variation due to nitrogen (between trays) is higher than variation within a tray. This means that nitrogen levels explain at least part of the variation in biomass that we observe. We can not conclude how different nitrogen levels affects the biomass of the different weed treatments. We also can not determine which nitrogen treatments resulted in the highest or lowest biomass. It is possible that there is an interaction between the nitrogen treatment and the weed treatment and more tests are needed to determine if this is the case.
...
biomass_1_specific_effects_bind <- rbind(biomass_1_specific_effects)
summary(biomass_1_specific_effects_bind, infer = c(T,T))

# plot(biomass_1_specific_effects_bind, horiz=F)
```
> Biomass for Weed_1 was not affected by the four different Nitrogen treatments. The Nitr_1 treatment had the highest biomass across weed treatments, but it was only distinguishable from Nitr_3 and Nitr_4 for both Weed_2 and Weed_3.
...
# # building a model to test the interaction effects of weed
# interaction_model <- lmer(perc_biomass ~ nitrogen + nitrogen:weed + (1|tray), data = biomass_1)
# anova(spec_eff_nitrogen_model)

# Is the effect of nitrogen greater for some weed species than others?
# Are there interaction effects? 
anova(biomass_1_model)

biomass_interaction <- contrast(variety_effects,method = 'pairwise')
summary(interaction_effects,infer = c(F,T))

nitrogen_effects_lab6 <- contrast(nitrogen_means, method = 'trt.vs.ctrl')
nitrogen_effects_lab6_bind <- rbind(nitrogen_effects_lab6)
summary(nitrogen_effects_lab6_bind,infer = c(T,T))
...
# needed?
cld(specific_effects_bind)

# .... do I need the above? below? both?
nitrogen_effects <- contrast(nitrogen_means, 'pairwise')
cld(nitrogen_means, level = 0.05/3)

```

> According to our ANOVA, there is strong evidence that there are interactions between nitrogen and weed species. There is a very low p-value and a large F value, meaning that the specific effects of nitrogen change between levels of weed.
